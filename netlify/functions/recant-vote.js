// /.netlify/functions/recant-vote.js
...
function normalizeAmount(value) {
  ...
  if (!Number.isInteger(amount) || amount < 1) {
    throw new Error('Recant amount must be a positive whole number.');
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { data, error } = await userSupabase.rpc('recant_vote_secure', {
      p_story_id: storyId,
      p_amount: amount
    });
    ...
    if (error) {
      ...
      return jsonResponse(400, {
        ...
        error: error.message || 'Could not recant vote.'
      });
    }
    ...
    if (!data?.success) {
      return jsonResponse(400, {
        success: false,
        reason: data?.reason || 'recant_failed',
        error: data?.message || 'Could not recant vote.',
        result: data || null
      });
    }
    ...
    console.error('recant-vote function error:', error);
